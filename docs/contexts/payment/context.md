...
**边界范围**: 基于需求文档2.1功能边界，专注于支付相关功能，与订单系统明确分工：支付单管理、支付渠道管理、支付执行、退款执行、对账管理、监控报警
...
### 主要职责
1. **支付单管理**: 接收订单系统创建的支付单请求，生成唯一支付单号，管理支付单的完整生命周期，包括状态跟踪、金额计算和业务规则验证
2. **支付渠道集成**: 统一集成多种支付渠道（线上支付、钱包支付、电汇支付、信用账户），提供标准化的支付接口和防腐保护
3. **统一支付处理**: 基于统一批量处理架构，支持单支付单和多支付单合并支付，支持部分支付和分批支付的灵活执行
4. **支付回调处理**: 处理支付渠道的异步回调通知，更新支付单状态和金额，考虑B2B大额支付的长确认时间特性
5. **退款执行**: 接收订单系统的退款指令，选择合适的支付流水执行退款操作，更新退款状态和金额
6. **信用还款处理**: 处理信用管理系统的还款支付单创建请求，按统一支付流程管理还款业务
7. **对账管理**: 与各支付渠道进行日常对账，处理差异和生成报表，确保交易一致性
