...
(
    ...
    refund_status           VARCHAR(20) NOT NULL DEFAULT 'NO_REFUND' COMMENT '退款状态',
    ...
    PRIMARY KEY (id)
) COMMENT '支付单表' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT = DYNAMIC;
...
(
    ...
    update_time                 DATETIME NULL COMMENT '更新时间',
    PRIMARY KEY (id)
) COMMENT '支付流水表' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT = DYNAMIC;
...
-- 示例：插入一个普通预付款支付单
INSERT INTO cms_payment (
    id, order_id, reseller_id, payment_amount, paid_amount, refunded_amount, actual_amount,
    currency, payment_type, payment_status, refund_status, business_desc,
    payment_deadline, priority_level, del_flag, create_by, create_by_name, create_time, update_time
) VALUES (
    'PAY20250115001', 'ORDER001', 'RESELLER001', 100000.000000, 0.000000, 0.000000, 0.000000,
    'CNY', 'ADVANCE_PAYMENT', 'UNPAID', 'NO_REFUND', '订单ORDER001的预付款',
    DATE_ADD(NOW(), INTERVAL 7 DAY), 1, 0, 'SYSTEM', '系统', NOW(), NOW()
);
...
INSERT INTO cms_payment (
    id, order_id, reseller_id, payment_amount, paid_amount, refunded_amount, actual_amount,
    currency, payment_type, payment_status, refund_status, business_desc,
    related_business_id, related_business_type, business_expire_date,
    payment_deadline, priority_level, del_flag, create_by, create_by_name, create_time, update_time
) VALUES (
    'PAY20250115002', NULL, 'RESELLER001', 50000.000000, 0.000000, 0.000000, 0.000000,
    'CNY', 'CREDIT_REPAYMENT', 'UNPAID', 'NO_REFUND', '信用记录CREDIT001的还款',
    'CREDIT001', 'CREDIT_RECORD', DATE_ADD(NOW(), INTERVAL 30 DAY),
    DATE_ADD(NOW(), INTERVAL 30 DAY), 2, 0, 'SYSTEM', '系统', NOW(), NOW()
);
